...
static bool MakeUserDirs()
{
   ...
   if (GetConfigPath(path, sizeof(path), "LIT/basic") <= 0)
      return false;
   if (!do_mkdir(path))
      return false;

   if (GetConfigPath(path, sizeof(path), "LIT/logs") <= 0)
      return false;
   if (!do_mkdir(path))
      return false;

   if (GetConfigPath(path, sizeof(path), "LIT/profiler_data") <= 0)
      return false;
   if (!do_mkdir(path))
      return false;

#ifdef _WIN32
   if (GetConfigPath(path, sizeof(path), "LIT/crashes") <= 0)
      return false;
   if (!do_mkdir(path))
      return false;

   if (GetConfigPath(path, sizeof(path), "LIT/updates") <= 0)
      return false;
   if (!do_mkdir(path))
      return false;
#endif

   if (GetConfigPath(path, sizeof(path), "LIT/plugin_config") <= 0)
      return false;
   if (!do_mkdir(path))
      return false;
...
static bool MakeUserProfileDirs()
{
   ...
   if (GetConfigPath(path, sizeof(path), "LIT/basic/profiles") <= 0)
      return false;
   if (!do_mkdir(path))
      return false;

   if (GetConfigPath(path, sizeof(path), "LIT/basic/scenes") <= 0)
      return false;
   if (!do_mkdir(path))
      return false;
...
{
   sleepInhibitor = os_inhibit_sleep_create("LIT Video/audio");
}
...
static void move_basic_to_profiles(void)
{
   ...
   if (GetConfigPath(path, 512, "LIT/basic") <= 0)
      return;
   if (!os_file_exists(path))
      return;

   /* if the profiles directory doesn't already exist */
   if (GetConfigPath(new_path, 512, "LIT/basic/profiles") <= 0)
      return;
...
static void move_basic_to_scene_collections(void)
{
   ...
   if (GetConfigPath(new_path, 512, "LIT/basic/scenes") <= 0)
      return;
   if (os_file_exists(new_path))
      return;
...
{
   BPtr<char>       logDir(GetConfigPathPtr("LIT/logs"));
   struct os_dirent *entry;
   os_dir_t         *dir        = os_opendir(logDir);
...
static void create_log_file(fstream &logFile)
{
   ...
   dst << "LIT/logs/" << currentLogFile.c_str();
...
#define LITERAL_SIZE(x) x, (sizeof(x) - 1)
   ostringstream dst;
   dst.write(LITERAL_SIZE("LIT/profiler_data/"));
...
static int run_program(fstream &logFile, int argc, char *argv[])
{
   ...
   try
   {
      ...
      delete_oldest_file("LIT/profiler_data");
      ...
      {
         blog(LOG_WARNING, "\n================================");
         blog(LOG_WARNING, "Warning: LIT is already running!");
         blog(LOG_WARNING, "================================\n");
      ...
      else if (already_running && multi)
      {
         blog(LOG_INFO, "User enabled --multi flag and is now "
              "running multiple instances of LIT.");
      }
...
static void main_crash_handler(const char *format, va_list args, void *param)
{
   ...
   delete_oldest_file("LIT/crashes");

   string name = "LIT/crashes/Crash ";
   ...
   int ret = MessageBoxA(NULL, CRASH_MESSAGE, "LIT has crashed!",
...
/* if using XDG and was previously using an older build of LIT, move config
...
static void move_to_xdg(void)
{
   ...
   if (snprintf(old_path, 512, "%s/.LIT", home) <= 0)
      return;
   ...
   if (GetConfigPath(new_path, 512, "LIT") <= 0)
      return;
...
{
   char path[512];
   int pathlen = GetConfigPath(path, 512, "LIT/basic/profiles");

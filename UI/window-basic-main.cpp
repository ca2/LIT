...
{
   ProfileScope("OBSBasic::InitLITCallbacks");
...
void OBSBasic::LITInit()
{
   ProfileScope("OBSBasic::LITInit");
   ...
   ret = snprintf(fileName, 512, "LIT/basic/scenes/%s.json",
                  sceneCollection);
   if (ret <= 0)
...
#define MAKE_CALLBACK(pred, method, log_action) \
   ...
   auto clipRecording = [](void *data, obs_hotkey_id,
                           obs_hotkey_t*, bool pressed)
   {
      OBSBasic * p = (OBSBasic*) data;
      if (pressed)
      {
         //  MessageBox(NULL, L"Clip Recording", L"Clip Recording", MB_ICONINFORMATION);

         p->ClipRecording();
      }
   };

   //MessageBox(NULL, L"Clip Recording1", L"Clip Recording1", MB_ICONINFORMATION);
   clipRecordingHotkey = obs_hotkey_register_frontend(
                         "OBSBasic.ClipRecording",
                         Str("Basic.Main.ClipRecording"),
                         clipRecording, this);

   LoadHotkey(clipRecordingHotkey, "OBSBasic.ClipRecording");
...
{
   char logDir[512];
   if (GetConfigPath(logDir, sizeof(logDir), "LIT/logs") <= 0)
      return nullptr;
...
inline void OBSBasic::OnDeactivate()
{
   if (!outputHandler->Active() && !ui->profileMenu->isEnabled())
   {
      ...
      if (trayIcon)
         trayIcon->setIcon(QIcon(":/res/images/lit.png"));
   }
}
...
{
   char path[512];
   int ret = GetConfigPath(path, 512, "LIT");
   if (ret <= 0)
      return;
...
int OBSBasic::GetProfilePath(char *path, size_t size, const char *file) const
{
   ...
   ret = GetConfigPath(profiles_path, 512, "LIT/basic/profiles");
   if (ret <= 0)
      return ret;
...
{
   trayIcon = new QSystemTrayIcon(QIcon(":/res/images/lit.png"),
                                  this);
   trayIcon->setToolTip("LIT Studio");
                             ...
                             QSystemTrayIcon::MessageIcon n)
{
   if (QSystemTrayIcon::supportsMessages())
   {
      ...
      QSystemTrayIcon::MessageIcon(n);
      trayIcon->showMessage("LIT Studio", text, icon, 10000);
   }
}
